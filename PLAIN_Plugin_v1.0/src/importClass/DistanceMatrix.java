...
import adpi.java.jadaptiveinterface.evaluate.views.CandidateEvaluate;
...
public class DistanceMatrix {
   ...
   /* public List<MoveMethodCandidateRefactoring> getMoveMethodCandidateRefactoringsByDistance() {
	   ...
	   public List<String> getExtractClassCandidateEvaluatee(Set<String> classNamesToBeExamined, IProgressMonitor monitor) {
	    	List<String> candidateList = new ArrayList<String>();
	    	//String ClassName = this.classNames[1] ;
	    	Iterator<MyClass> classIt = system.getClassIterator();
	    	
	    	while(classIt.hasNext()) {
	    		MyClass myClass = classIt.next();
	    		if(classNamesToBeExamined.contains(myClass.getName())) {
	    			candidateList.add(myClass.getName());
	    			MyClass currentSuperclass = classList.get(classIndexMap.get(classNamesToBeExamined));
	    	}	}
		   return candidateList;}
	   private boolean CandidateClass(String targetClass, Set<String> candidateTargetClasses) {
	    	for(String candidateTargetClass : candidateTargetClasses) {
	    		if(!candidateTargetClass.equals(targetClass)) {
	    			MyClass currentSuperclass = classList.get(classIndexMap.get(candidateTargetClass));
	    			String superclass = null;
	    ...
	    public List<MyClass> getExtractClassCandidateEvaluate (Set<String> classNamesToBeExamined, IProgressMonitor monitor) {
	    	List<MyClass> candidateList = new ArrayList<MyClass>();
	    	double entityPlacement0 = this.getSystemEntityPlacementValue();
	    	Iterator<MyClass> classIt = system.getClassIterator();
